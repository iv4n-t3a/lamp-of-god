  ...
  if (offset_index_ < std::size(offsets_) and
    ...
    for (int i = 0; i < height_ - 1; i++) {
      ...
      for (int j = 0; j < width_ - 1; j++) {
        potentials.back().push_back(Vector(j * tube_width / (width_ - 1),
                                           i * tube_height / (height_ - 1)));
      }
      potentials.back().push_back(
          Vector(tube_width, i * tube_height / (height_ - 1)));
    }
    potentials.push_back(std::vector<Vector<potential_t>>());
    for (int j = 0; j < width_ - 1; j++) {
      potentials.back().push_back(
          Vector(j * tube_width / (width_ - 1), tube_height));
    }
...
void AveragePotentialObserver::NewFrame(delay_t delta_time) {
  ...
  for (int i = 0; i < height_ - 1; i++) {
    for (int j = 0; j < width_ - 1; j++) {
      potentials_[i][j] += Vector(j * tube_width / (width_ - 1),
                                  i * tube_height / (height_ - 1));
    }
    potentials_[i].back() +=
        Vector(tube_width, i * tube_height / (height_ - 1));
